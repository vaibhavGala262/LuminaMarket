...
import { productAPI, cartAPI, searchAPI, getSessionId } from './services/api.js';
...
const App = () => {
  ...
  const addToCart = async (product) => {
    try {
      await cartAPI.addItem(sessionId, product.id || product._id, 1);
  ...
  const updateQuantity = async (id, delta) => {
    try {
      ...
      await cartAPI.updateItem(sessionId, id, newQuantity);
      await fetchCart(); // Refresh cart
    } catch (err) {
  ...
  const removeFromCart = async (id) => {
    try {
      await cartAPI.removeItem(sessionId, id);
      await fetchCart(); // Refresh cart
    } catch (err) {
                      ...
                      {[4, 3, 2, 1].map((stars) => (
                        <button
                          ...
                          className={`flex items-center w-full px-3 py-2 rounded-lg text-sm transition-colors ${
                            minRating === stars ? 'bg-slate-100 font-semibold text-slate-900' : 'text-slate-600 hover:bg-slate-50'
                          }`}
                        >
